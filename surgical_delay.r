###################################################################################
##                                                                               ##
##   This R script performs the surgical  procedure approach to assess           ##
##   the potential patient outcomes associated with surgical procedure delay.    ##
##                                                                               ##
...
cdmSchema = "cdm_v5"
dbms = "netezza" # Should be "sql server", "oracle", "postgresql" , "redshift" or "netezza"
...
# Creating table of filtered procedures (removing some procedures that do not provide much information)
executeSql(conn, 
           "drop table proc_filtered if exists;
            ...
            --surgical
            and regexp_like(concept_code, '^[1-6][0-9][0-9][0-9][0-9]$', 'i')
            ...
            and not regexp_like(concept_name, '(monitoring device)|(Infusion Device)|(Endotracheal Airway)', 'i')
            and regexp_like(concept_code, '^0', 'i'))
            ")
...
count$TOTAL_2020[is.na(count$TOTAL_2020)] <- median(na.omit(count$TOTAL_2020))
count$COUNT_2018[is.na(count$COUNT_2018)] <- 0
count$TOTAL_2018[is.na(count$TOTAL_2018)] <- median(na.omit(count$TOTAL_2018))
...
# Pulling final procedure delay dataframe where procedure delay 
...
# Creating table for readmission within 30, 90, 180 days (inpatient or emergency)
executeSql(conn,
           ...
           max(case when visit_start_date between b.visit_end_date + 1 and b.visit_end_date + 180 then 1 else 0 end) as readmin_180
...
outcomes <- querySql(conn,
                     ...
                     case when death_date between h.proc_date and h.proc_date + 180 then 1 else 0 end as death_180,
                     readmin_180
...
#############################################################################
#                Grabbing demographic and covariates                        #
#############################################################################
...
proc$READMIN_180[is.na(proc$READMIN_180)] <-  0
...
proc_gems <- proc_gems %>% 
  group_by(PERSON_ID, GRID, SEX, RACE, YEAR_OF_BIRTH, 
           ...
           PROC_DATE, 
           CONCEPT_CODE_D, CONCEPT_NAME_D,
           HOSP_LENGTH, HOSP_DEATH, READMIN_180) %>% 
...
# Adjusting procedure delay to months
proc$PROC_DELAY <- proc$PROC_DELAY / (365.25/12)
...
pairs <- proc %>% distinct(ICD, CONCEPT_CODE_D)
...
results <- results[, c('ICD', 'ICD10CM_STR','PROC_CODE', 'PROC_STR', 
                       'N', 'OUTCOME', 
                       'BETA_OR', 'CI_LOWER', 'CI_UPPER', 'P')]
